...
class MealFood {
  ...
  static timesEaten () {
    ...
    //                      JOIN foods ON meal_foods.food_id = foods.id GROUP BY meal_foods.food_id, foods.name, foods.id;`)
      return database.raw(`SELECT count(foods.id) as timeseaten, foods.name, foods.calories FROM meal_foods JOIN foods ON meal_foods.food_id = foods.id GROUP BY meal_foods.food_id, foods.name, foods.calories ORDER BY timesEaten DESC;`)
      // return database.raw(`SELECT count(meal_foods.food_id) as timesEaten,
